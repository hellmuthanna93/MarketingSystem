...
There is a large gap between a **free** discovery call and a longer counseling
package or The Next Chapter. Many ideal clients are insight-rich
but cautious; they want to *feel* the depth before committing. The Clarity
Session is the proposed missing rung.
...
```mermaid
flowchart LR
    lead["Lead (inbound / discovery call)"] --> wedge["Clarity Session (paid)"]
    wedge --> summary["Written pattern summary (72h)"]
    summary --> followup["Day-7 follow-up email"]
    followup -->|Ready to go deeper| package["Counseling or coaching package"]
    followup -->|Not now| nurture["Stays in audience / nurture"]
```
