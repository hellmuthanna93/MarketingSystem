...
```mermaid
flowchart LR
    content[Consistent depth content] --> trust[Recognition and trust]
    trust --> follow[Follow / subscribe]
    follow --> discovery[Free discovery call]
    discovery --> wedge["Clarity Session (wedge)"]
    discovery --> package[Package]
    wedge --> package
```
