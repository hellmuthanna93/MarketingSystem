...
```mermaid
flowchart LR
    decide[Client decides to start] --> confirm[Confirmation + payment]
    confirm --> agreement[Agreement + intake form]
    agreement --> schedule[Schedule first session]
    schedule --> prep[Pre-session reminder]
    prep --> first[First session]
```
