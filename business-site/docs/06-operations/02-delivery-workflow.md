...
```mermaid
flowchart LR
    inquiry[Inbound inquiry] --> discovery[Discovery call]
    discovery --> recommend[Recommendation]
    recommend --> onboard[Onboard: agreement + intake + payment]
    onboard --> sessions[Sessions]
    sessions --> notes[Post-session notes]
    sessions --> between[Between-session work / assignments]
    sessions --> close[Completion]
    close --> renewal[Renewal / referral / testimonial]
```
