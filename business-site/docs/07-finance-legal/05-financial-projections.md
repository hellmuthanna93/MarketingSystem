...
```mermaid
flowchart LR
    consistency[Consistent content] --> pipeline[Predictable discovery calls]
    pipeline --> wedge[Wedge conversions]
    wedge --> packages[More coaching packages]
    price[Price increases] --> revenue[Higher revenue per client]
    packages --> revenue
    audience[Owned audience] --> pipeline
```
